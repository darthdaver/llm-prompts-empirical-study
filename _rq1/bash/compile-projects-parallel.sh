...
while (( current_index < num_files )); do
    if (( ${#running_pids[@]} < max_jobs )); then
        nohup bash "${RQ1_DIR}/bash/compile-projects.sh" "${RESOURCES_DIR}/github-repos-split/split_${current_index}.csv" "split_${current_index}.csv" "split_${current_index}.csv" > "logs/log-${current_index}.out" 2>&1 &
        last_pid=$!
