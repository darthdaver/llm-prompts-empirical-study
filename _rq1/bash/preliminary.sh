...
while IFS=, read -r ol_model hf_tokenizer num_ctx model_type; do
  for config_num in "${RQ1_DIR}/queries/preliminary"/*/; do
    config_num=$(basename "$config_num")
    # Clean repo_name string from undesired white-spaces/line-breaks introduced with the CSV parsing
    model_type="${model_type//[$'\t\r\n ']/}"
    for i in {1..3}; do
      i=$(basename "$i")
      echo "Running inference with ${ol_model} on query template ${config_num} - ${i}"
      "${PY_ENV}" "${RQ1_DIR}/preliminary.py" \
          --model_name_or_path "${ol_model}" \
          --model_type "${model_type}" \
          --tokenizer_name "${hf_tokenizer}" \
          --query_path "${QUERIES_DIR}/preliminary/${config_num}/query-template-${i}.txt" \
          --dataset_path "${OUTPUT_PROMPT_DIR}/preliminary-prompt" \
          --output_path "${OUTPUT_DIR}/preliminary/${ol_model}-${config_num}-${i}" \
          --src_col "src" \
          --tgt_col "tgt" \
          --num_ctx "${num_ctx}" \
          --ram_saving true
