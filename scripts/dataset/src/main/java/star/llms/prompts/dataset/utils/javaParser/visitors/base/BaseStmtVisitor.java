...
/**
 * The class visit a node (and all its descendants within the AST) and collects all the nodes <S>
 * that extends {@link com.github.javaparser.ast.stmt.Statement} found.
 *
 */
public abstract class BaseStmtVisitor<S extends Statement> extends VoidVisitorAdapter<List<S>> {

    public List<S> visit(Statement statement) {
        List<S> collection = new java.util.ArrayList<>();
    ...
    public List<S> visit(MethodDeclaration methodDeclaration) {
        List<S> collection = new java.util.ArrayList<>();
        visit(methodDeclaration.getBody().get(), collection);
        return collection;
    }

    // Generic method to add the statement to the collection
    protected void addToCollection(S stmt, List<S> collection) {
        collection.add(stmt);
    }
}